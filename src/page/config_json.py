...
def show_page() -> None:
    ...
    json_file = st.file_uploader(
        "Escolha um arquivo JSON de credenciais do Firebase", type="json"
    )
    ...
    if json_file is not None:
        try:
            ...
            if json_data.get("type") == "service_account":
                ...
                st.error(
                    'O certificado de conta de serviço é inválido. '
                    'O certificado deve conter um campo "type" definido '
                    'como "service_account".'
                )
        except json.JSONDecodeError as e:
            st.error(f"Erro ao carregar o arquivo JSON: {e}")
    else:
        st.warning("Por favor, carregue um arquivo JSON de credenciais do "
                    "Firebase.")
